...
Django-CFG automatically configures security based on environment mode (`IS_PROD`, `IS_DEV`, `IS_TEST`):

**Development Mode (`IS_DEV=true`):**
- CORS: Whitelist mode with common dev ports
- CSRF: common local dev ports; CORS allows any localhost port via regex
- SSL: Disabled (local development)
- Credentials: Allowed for frontend testing
- DEBUG: Can be enabled for detailed error pages
...
- **DEBUG: Automatically disabled** (even if `DEBUG=true` is set)

<Callout type="warning">
**Production Security Enforcement**

In production mode (`IS_PROD=true`), Django-CFG **automatically disables DEBUG** regardless of the `DEBUG` environment variable. This prevents:
- Sensitive data exposure in error pages
- Memory leaks from SQL query logging
- Performance degradation

If you need debug logging in production, use proper logging configuration or error tracking services (Sentry, Logfire) instead.
</Callout>
...
> **Note:** `CORS_ALLOW_ALL_ORIGINS = True` doesn't support credentials, so Django-CFG never sets it. Production uses a whitelist; development adds regexes covering any `localhost` port.
...
- **[Environment Setup](./environment-setup)** - Environment configuration
