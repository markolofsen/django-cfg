...
- [DjangoConfig reference](../../configuration/django-settings)
...
**Solution:**
```bash
# Make sure django-cfg is installed (Python 3.12+ required)
...
<Callout type="info">
**Multi-Database Migration Control**
...
See [Multi-Database Guide](./multi-database) for detailed patterns.
...
- ✅ `localhost`, `127.0.0.1`, `[::1]` on **any port**, http or https, via `CORS_ALLOWED_ORIGIN_REGEXES`
- ✅ `CORS_ALLOW_CREDENTIALS = True` - cookie auth works (a `*` wildcard would break it)
- ✅ `ALLOWED_HOSTS = ['*']` - All hosts accepted
- ✅ Docker IPs work automatically — CORS does not apply to server-to-server calls
...
<details>
<summary>**A browser request is blocked by CORS in production**</summary>

**Problem:**
The calling origin is not in `CORS_ALLOWED_ORIGINS`.

**Cause:**
Production builds the whitelist from `security_domains`, and the origin is missing.
...
<Callout type="error">
**Security Critical: CORS Misconfiguration**
**Django-CFG behavior:**
- ✅ **Development** (`env_mode="development"`) → localhost on any port, via regex
- 🔒 **Production** (`env_mode="production"`) → Strict whitelist only
...
from django_cfg import DjangoConfig, StartupInfoMode

# Development
class DevelopmentConfig(DjangoConfig):
    debug: bool = True
    startup_info_mode: str = StartupInfoMode.FULL

# Production
class ProductionConfig(DjangoConfig):
    debug: bool = False
    startup_info_mode: str = StartupInfoMode.NONE
    security_domains: list[str] = ["example.com"]
```
...
- [Architecture Guide](../../core/architecture)
