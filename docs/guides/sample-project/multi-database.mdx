...
Set connection URLs via environment variables:

```bash
# Default database
DATABASE__URL="postgresql://djangocfg:password@db.example.com:5432/djangocfg_main"

# Blog database
BLOG_DATABASE__URL="postgresql://djangocfg_blog:password@blog-db.example.com:5432/djangocfg_blog"

# Shop database
SHOP_DATABASE__URL="postgresql://djangocfg_shop:password@shop-db.example.com:5432/djangocfg_shop"
```

Then reference them in `config.py`:

```python
databases: Dict[str, DatabaseConfig] = {
    "default": DatabaseConfig.from_url(url=env.database.url),
    "blog_db": DatabaseConfig.from_url(
        url=env.blog_database.url,
        apps=["apps.blog"],
    ),
    "shop_db": DatabaseConfig.from_url(
        url=env.shop_database.url,
        apps=["apps.shop"],
        operations=["read", "write", "migrate"],
    ),
}
...
Health check endpoint (using standard Django db connections):

```python
from django.db import connections

def check_all_databases():
    """Check all configured database aliases."""
    results = {}
    for alias in connections:
        try:
            connections[alias].ensure_connection()
            results[alias] = {"status": "healthy"}
        except Exception as e:
            results[alias] = {"status": "unhealthy", "details": str(e)}
    return results
```
