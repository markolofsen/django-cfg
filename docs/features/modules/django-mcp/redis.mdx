...
Persists the conversation history for the admin chat agent between page reloads. `agents/api/views.py` reads the history into the agent context before each run and appends the user message and the assistant reply after it, keyed on `request.user.pk` plus the request's `session_id`. `GET /cfg/mcp/agent/history/` returns the stored messages for a session.
