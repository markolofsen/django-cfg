...
### `GET /cfg/mcp/agent/history/?session_id=...`

Returns `{"messages": [...]}` — the stored conversation for the calling user and that session, oldest first. `session_id` defaults to `"agent-session"`. Requires an authenticated Django session; an anonymous request gets `401`.

<Callout type="info">
The agent endpoints are mounted only on the module's own `endpoint_path`, never on a declared [profile](/features/modules/django-mcp/configuration#profiles) path. They spend an LLM budget on the deployment's account, so mounting them on an anonymous profile would make it a public spending endpoint.
</Callout>
...
every registered MCPTool → a pydantic_ai.Tool, via Tool.from_schema(input_schema)
