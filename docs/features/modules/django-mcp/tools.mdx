...
- `is_available(context)` may be called with `context=None` — the public `/info/` listing checks availability without a request context. If you override `is_available`, it must tolerate a missing context rather than assume one is always present.
