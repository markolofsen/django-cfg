...
class AdminConfig(BaseModel):
    ...
    # Filters and search
    # Accepts List[FilterSpec] where FilterSpec = str | type | tuple[str, type]
...
TypeAlias for `list_filter` entries. Accepts a plain string field name, a filter class, or a `(field_name, filter_class)` tuple.

```python
from django_cfg.modules.django_admin import FilterSpec

FilterSpec = str | type | tuple[str, type]

# Examples:
list_filter: List[FilterSpec] = [
    "status",                          # str — field name
    MyCustomFilter,                    # type — filter class
    ("created_at", DateRangeFilter),   # tuple — (field, filter class)
]
```
