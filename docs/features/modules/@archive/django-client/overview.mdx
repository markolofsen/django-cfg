...
The standalone `django_client` module has been merged into `django_generator`. Client generation is now part of the code generation pipeline. Use `from django_cfg.modules.django_generator.public.config import Config, OpenAPI, Target, Language` for programmatic access.
</Callout>
