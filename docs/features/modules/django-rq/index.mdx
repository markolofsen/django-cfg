...
> **📚 Part of**: [Integrations](/features/modules/django-rq) - Explore all django-cfg integrations
...
<Callout type="info">
**D1 Persistence:** Job events and worker heartbeats are persisted to Cloudflare D1 for historical analysis. Redis handles real-time state only — no PostgreSQL tables required for RQ operation.
</Callout>
...
- **CLI commands**: `rqworker`, `rqscheduler`, `rqstats`, `rq_cleanup_locks`
...
Django-RQ is **already included** in django-cfg via the `rq` optional dependency group. Enable it:

```bash
pip install "django-cfg[rq]"
# Installs: django-rq>=4.0.0, rq>=2.7.0, rq-scheduler>=0.14.0, redis>=5.0
```
