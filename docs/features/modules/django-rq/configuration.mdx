...
See [Redis Isolation](#redis-isolation) for details.
:::
...
1. **Cache Flush Kills Jobs**: Any project clearing cache deletes scheduler job metadata
2. **Shared Keys**: All schedulers read from the same `rq:scheduler:scheduled_jobs` set
3. **Job Theft**: Workers from one project try to execute jobs from another
4. **Missing Functions**: Jobs fail because task functions don't exist in other projects
5. **Duplicate Execution**: Same job runs multiple times across projects
...
1. `DjangoConfig.redis_url` provides the base Redis connection (e.g., `redis://localhost:6379/0`)
2. `DjangoRQConfig.redis_db` **replaces** the database number in the URL
3. All queues, workers, and schedulers use the isolated database
...
# Resulting URL: redis://shared-redis:6379/5
```
...
DjangoRQConfig(redis_db=16, ...)  # Error: must be <= 15
```
...
### Documentation
- [Overview](/features/modules/django-rq) - Introduction and features
- [Architecture](./architecture) - System design
- [Examples](./examples) - Real-world examples
- [Monitoring](./monitoring) - Monitoring and management

### Reference
- [Django-RQ Docs](https://github.com/rq/django-rq) - Official documentation (v4+)
- [RQ Docs](https://python-rq.org/) - Core RQ documentation (v2.7+)
- [RQ Scheduler](https://github.com/rq/rq-scheduler) - Scheduler documentation (v0.14+)
- [RQ Changelog](https://github.com/rq/rq/blob/master/CHANGES.md) - Breaking changes in RQ 2.x
