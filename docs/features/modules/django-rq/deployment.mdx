...
<Callout type="error">
**Critical: RQ Scheduler Queue Configuration**

The `rqscheduler` command only processes jobs for **ONE queue** (default: `default`).

If your scheduled tasks use `queue="high"` in `RQScheduleConfig`, you **MUST** run:
```bash
python manage.py rqscheduler --queue high
```

**Best practice:** Put ALL scheduled tasks (`RQScheduleConfig`) on the same queue (recommended: `high`) and run the scheduler with that queue. Workers listen to all queues, so they will execute the jobs regardless of which queue they're on.
...
services:
  ...
  # RQ Scheduler
  # IMPORTANT: --queue must match the queue used in RQScheduleConfig (api/config.py)
  # rqscheduler only processes ONE queue. All scheduled tasks should use the same queue.
  rq-scheduler:
    image: your-registry/django-app:latest
...
services:
  ...
  # IMPORTANT: --queue must match the queue used in RQScheduleConfig
  # rqscheduler only processes ONE queue
  rq-scheduler:
    build: .
    command: python manage.py rqscheduler --queue high
    env_file: .env.local
...
# IMPORTANT: --queue must match the queue used in RQScheduleConfig
scheduler: python manage.py rqscheduler --queue high
```
...
class ProductionConfig(DjangoConfig):
    ...
    django_rq: DjangoRQConfig = DjangoRQConfig(
        enabled=True,
        redis_db=0,  # REQUIRED: unique per project (0-15) for isolation
        queues=[
        ...
        # IMPORTANT: All schedules must use the same queue!
        # rqscheduler only processes ONE queue (specified with --queue flag)
        schedules=[
            RQScheduleConfig(
                func="apps.crypto.tasks.update_coin_prices",
                interval=300,  # Every 5 minutes
                queue="high",  # Must match rqscheduler --queue
            ),
        ],
...
4. **Connection Pooling**: Use Redis connection pool (auto-enabled). Workers
   run **without** Django's psycopg pool: every job is a forked process, and a
   pool inherited across `fork()` cannot open connections. `rqworker` removes
   it automatically — pool at PgBouncer or the server instead.
...
| **Scheduled jobs not executing** | **Queue mismatch** | **Ensure `rqscheduler --queue X` matches `RQScheduleConfig(queue="X")`** |
| **Scheduler hangs on startup** | **Stale locks in Redis** | **Automatic cleanup built-in, or run `python manage.py rq_cleanup_locks`** |

### Stale Scheduler Locks

If the scheduler container crashes without proper cleanup, stale lock keys (`rq:scheduler_instance:*`) may remain in Redis and prevent new schedulers from starting.

**Django-CFG handles this automatically** - the `rqscheduler` command cleans up stale locks before starting.

For manual cleanup:

```bash
# Check for stale locks (dry run)
