...
description: Complete guide to setting up Centrifugo WebSocket integration with native RPC proxy in your Django-CFG project.
---
...
class MyConfig(DjangoConfig):
    ...
    centrifugo: Optional[DjangoCfgCentrifugoConfig] = DjangoCfgCentrifugoConfig(
        ...
        # Python Wrapper URL (required)
        wrapper_url="http://centrifugo-wrapper:8080",

        # Centrifugo server URLs
        centrifugo_url="ws://localhost:8002/connection/websocket",
        centrifugo_api_url="http://centrifugo:8002/api",
        centrifugo_api_key="your-api-key-min-32-chars",
        centrifugo_token_hmac_secret="your-hmac-secret-min-32-chars",
...
CENTRIFUGO__WRAPPER_URL=http://centrifugo-wrapper:8080
CENTRIFUGO__CENTRIFUGO_URL=ws://localhost:8002/connection/websocket
CENTRIFUGO__CENTRIFUGO_API_URL=http://centrifugo:8002/api
...
CENTRIFUGO__ACK_TIMEOUT=10
CENTRIFUGO__LOG_LEVEL=INFO
CENTRIFUGO__LOG_ALL_CALLS=false
```
...
from django_cfg.modules.django_centrifugo.decorators import websocket_rpc
...
### DjangoCfgCentrifugoConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | `bool` | `False` | Enable Centrifugo integration |
| `wrapper_url` | `str` | `"http://localhost:8080"` | Python Wrapper HTTP API URL |
| `wrapper_api_key` | `Optional[str]` | `None` | Optional API key for wrapper authentication |
| `centrifugo_url` | `str` | `"ws://localhost:8002/connection/websocket"` | WebSocket URL for browser clients |
| `centrifugo_api_url` | `str` | `"http://localhost:8002/api"` | HTTP API URL for server-to-server calls |
| `centrifugo_api_key` | `Optional[str]` | `None` | API key for server-to-server authentication |
| `centrifugo_token_hmac_secret` | `Optional[str]` | `None` | HMAC secret for JWT generation (uses Django `SECRET_KEY` if `None`) |
| `default_timeout` | `int` | `30` | Default publish timeout in seconds |
| `ack_timeout` | `int` | `10` | ACK timeout for delivery confirmation in seconds |
| `http_timeout` | `int` | `35` | HTTP request timeout to wrapper in seconds |
| `max_retries` | `int` | `3` | Maximum retry attempts for failed publishes |
| `retry_delay` | `float` | `1.0` | Delay between retries in seconds |
| `verify_ssl` | `bool` | `False` | Verify SSL certificates for HTTPS connections |
| `log_all_calls` | `bool` | `False` | Log all publish calls to database |
| `log_only_with_ack` | `bool` | `True` | Only log calls that wait for ACK |
...
**Solution**:
1. Check Centrifugo is running: `curl http://localhost:8000/healthz`
