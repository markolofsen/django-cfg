...
API keys are stored as SHA-256 hashes instead of plaintext. The raw key is shown only once during creation - store it securely!
</Callout>
...
```python
from django_cfg.modules.django_grpc.models import GrpcApiKey

# Generate a new API key (returns tuple)
api_key, raw_key = GrpcApiKey.generate(
    name="My Service",
    user=user,  # Optional: associate with Django user
)

# IMPORTANT: raw_key is only available during creation!
print(f"Save this key: {raw_key}")  # Store securely
print(f"Key prefix: {api_key.key_prefix}")  # For identification
```

<Callout type="warning">
The raw key is only returned during generation. Store it securely - it cannot be retrieved later.
</Callout>

**Via Django Admin:**

1. Go to `/admin/` → **gRPC API Keys**
2. Click "Add gRPC API Key" or use "Generate New Key" action
...
api_key = GrpcApiKey.validate(incoming_key)
if api_key:
    print(f"Valid key: {api_key.name}")
    print(f"User: {api_key.user}")
else:
    print("Invalid or inactive key")

# Async validation (recommended for gRPC services)
api_key = await GrpcApiKey.avalidate_key(incoming_key)
if not api_key:
...
**Next Steps:**
- **[Configuration](./configuration)** - Complete auth config reference
- **[Getting Started](./getting-started)** - Build authenticated service
- **[FAQ](./faq)** - Authentication troubleshooting
