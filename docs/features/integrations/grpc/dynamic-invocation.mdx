...
# Or use ClientChannelConfig for full control
...
# Channel configuration with TLS
config = ClientChannelConfig(
...
client = DynamicGRPCClient(config=config, tls_config=tls_config)
```

## 🔁 Resilient Dynamic Client

For production use, combine dynamic invocation with resilience features:

### Sync Client with Resilience
...
# Standard dynamic client
client = DynamicGRPCClient(host="localhost", port=50051)

# Resilient client for production
...
# Use global pool for multiple clients
...
- **[Configuration](../configuration)** - gRPC configuration options
- **[Architecture](../architecture)** - gRPC system architecture

---

**Next:** Learn about [resilience patterns](../resilience) for production-grade gRPC.
