...
Access the admin interface at `/admin/integrations/grpc/grpcrequestlog/`:
...
Access monitoring data via REST API at `/cfg/integrations/grpc/monitor/`:

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/cfg/integrations/grpc/monitor/overview/` | GET | Overall statistics and health |
| `/cfg/integrations/grpc/monitor/requests/` | GET | Recent request logs (paginated) |
| `/cfg/integrations/grpc/monitor/services/` | GET | Per-service statistics |
| `/cfg/integrations/grpc/monitor/methods/` | GET | Per-method statistics |
| `/cfg/integrations/grpc/monitor/timeline/` | GET | Timeline data (hourly breakdown) |
| `/cfg/integrations/grpc/monitor/health/` | GET | Health check status |

### 1. Overview Endpoint

**GET** `/cfg/integrations/grpc/monitor/overview/`
...
**GET** `/cfg/integrations/grpc/monitor/requests/`
...
{
  "count": 1543,
  "next": "/cfg/integrations/grpc/monitor/requests/?page=2",
  "previous": null,
  "results": [
    {
      "request_id": "a3b4c5d6-e7f8-9012-3456-789abcdef012",
...
**GET** `/cfg/integrations/grpc/monitor/services/`
...
**GET** `/cfg/integrations/grpc/monitor/timeline/`
...
**GET** `/cfg/integrations/grpc/monitor/health/`
...
# Get overview
response = requests.get("http://localhost:8000/cfg/integrations/grpc/monitor/overview/")
data = response.json()
...
response = requests.get(
    "http://localhost:8000/cfg/integrations/grpc/monitor/requests/",
...
fetch('/cfg/integrations/grpc/monitor/services/?hours=24')
  .then(res => res.json())
  .then(data => {
...
// Get timeline data
fetch('/cfg/integrations/grpc/monitor/timeline/?hours=24')
  .then(res => res.json())
  .then(data => {
...
```bash
# Get overview
curl http://localhost:8000/cfg/integrations/grpc/monitor/overview/

# Get recent errors
curl "http://localhost:8000/cfg/integrations/grpc/monitor/requests/?status=error&hours=1"

# Get service statistics
curl http://localhost:8000/cfg/integrations/grpc/monitor/services/

# With authentication (if required)
curl -H "Authorization: Bearer <token>" \
  http://localhost:8000/cfg/integrations/grpc/monitor/overview/
```
