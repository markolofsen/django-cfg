...
1. **Shared Keys**: All schedulers read from the same `rq:scheduler:scheduled_jobs` set
2. **Job Theft**: Workers from one project try to execute jobs from another
3. **Missing Functions**: Jobs fail because task functions don't exist in other projects
4. **Duplicate Execution**: Same job runs multiple times across projects
