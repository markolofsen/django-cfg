...
```yaml title="docker-compose-production.yaml"
version: '3.8'
...
<Tabs groupId="compose-commands">
  <TabItem value="all" label="All Services" default>
...
```
  ...
  <TabItem value="scale" label="Scale Workers">
...
```
  ...
  <TabItem value="logs" label="View Logs">
...
```yaml title="docker-compose-local-services.yml"
services:
  redis:
    image: redis:7-alpine
    ports:
...
```
  ...
  <TabItem value="honcho" label="Using Honcho/Foreman">

Create `Procfile`:

```procfile title="Procfile.dev"
...
<Tabs groupId="k8s-resources">
  <TabItem value="deployment" label="Deployments" default>

```yaml title="k8s/django-rq.yaml"
apiVersion: apps/v1
kind: Deployment
metadata:
  name: django-api
...
```
  ...
  <TabItem value="service" label="Services">

```yaml title="k8s/services.yaml"
apiVersion: v1
kind: Service
metadata:
  name: django-api
...
```
  ...
  <TabItem value="config" label="ConfigMap">
...
<Tabs groupId="aws-deployment">
  <TabItem value="ecs" label="ECS Task Definition" default>

```json title="ecs-task-definition.json"
{
  "family": "django-rq",
  "networkMode": "awsvpc",
...
```
  ...
  <TabItem value="redis" label="ElastiCache Setup">
...
```yaml title="cloudrun-worker.yaml"
...
<Tabs groupId="env-files">
  <TabItem value="prod" label="Production (.env.prod)" default>
...
```
  ...
  <TabItem value="local" label="Local (.env.local)">
...
<Tabs groupId="health-checks">
  <TabItem value="django" label="Django API" default>
...
```
  ...
  <TabItem value="worker" label="RQ Worker">
...
<Tabs groupId="troubleshooting">
  <TabItem value="check-redis" label="Check Redis" default>
...
```
  ...
  <TabItem value="check-queue" label="Check Queue">
...
```
  ...
  <TabItem value="check-logs" label="Check Logs">
...
- **Production Ready?** Review [monitoring guide](./monitoring.md)
- **Need Examples?** Check [examples guide](./examples.md)
- **Advanced Config?** See [configuration reference](./configuration.md)
- **Architecture?** Read [architecture guide](./architecture.md)
