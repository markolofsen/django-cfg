...
class MyConfig(DjangoConfig):
    @property
    def django_rq(self) -> DjangoRQConfig | None:
        # Only run background workers outside of development
        if self.is_development:
            return None
        return DjangoRQConfig(enabled=True, redis_db=0)
```
...
### 2. Enable Only What Each Environment Needs
...
2. Review [Integration Patterns](./patterns) for best practices
