...
Schema generation fails with import errors when running `manage.py generate_client`.
...
```bash
.venv/bin/python manage.py generate_client
```
...
3. Report issues at [django-cfg issues](https://github.com/markolofsen/django-cfg/issues)
