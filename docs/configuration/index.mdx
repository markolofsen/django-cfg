...
- [**Media Storage**](./storage) - S3-compatible object storage (Cloudflare R2, MinIO, AWS S3)
...
- [**Telegram**](/features/modules/django-telegram) - Telegram bot configuration

### Security

- [**Two-Factor Authentication**](/features/built-in-apps/user-management/accounts/two-factor) - TOTP-based 2FA configuration

### Django Integrations

- [**Admin & UI**](/features/modules/django-unfold) - Unfold admin interface
...
# Use ${VAR_NAME:default_value} to provide a fallback
secret_key: "${SECRET_KEY}"
debug: false
project_name: "My Project"

database:
  url: "${DATABASE_URL:sqlite:///db.sqlite3}"
...
# Instantiate your config class directly
config = MyProjectConfig()
```
...
<Callout type="info">
**YAML Best Practice**
Keep sensitive values in environment variables and reference them in YAML with `${VAR_NAME}` syntax. Use `${VAR_NAME:default_value}` to provide a fallback when the variable is not set.
...
class MyConfig(DjangoConfig):
    debug: bool = False  # Set via env or override in subclass
