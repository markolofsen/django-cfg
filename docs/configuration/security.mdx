...
`CSRF_TRUSTED_ORIGINS` is still an explicit list of common dev ports — Django
compares it literally and supports no regex. Add yours to `security_domains` if
you run a dev server on an unusual port and hit a CSRF failure.
