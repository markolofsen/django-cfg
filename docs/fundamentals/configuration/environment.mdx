...
**Production Security: DEBUG Enforcement**

When `IS_PROD=true` is set, Django-CFG **automatically disables DEBUG** even if `DEBUG=true` is in your environment. This is a security feature that prevents:
- Exposure of sensitive data in error pages
- Memory leaks from SQL query logging
- Performance issues in production

**Always set `IS_PROD=true` in production environments** to enable this protection.
</Callout>
...
- [Docker Configuration](/docs/guides/docker/configuration)
- [Production Deployment](/docs/deployment/overview)
