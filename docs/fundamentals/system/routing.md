...
Django-CFG provides **intelligent routing** for databases, URLs, and dashboards—**zero manual configuration** needed!
:::

The **Routing System** provides intelligent database routing, automatic URL management, and dashboard callback integration for Django-CFG projects.
...
- **Dashboard Callbacks** - Dynamic admin dashboard content
...
Django-CFG provides a default dashboard callback for admin interfaces:
...
def dashboard_callback(request: HttpRequest, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Adds system metrics and quick actions to dashboard.
    """
...
Create custom dashboard callbacks:

```python
def business_metrics_callback(request: HttpRequest, context: Dict[str, Any]) -> Dict[str, Any]:
    """Custom callback with business metrics."""
    ...
    business_cards = [
        {
            ...
            "value": str(orders_this_week),
            "description": f"Total: {total_orders}",
            "icon": "shopping_cart",
            "color": "warning"
        },
        {
    ...
    context.setdefault("cards", []).extend(business_cards)
...
class MyProjectConfig(DjangoConfig):
    unfold: UnfoldConfig = UnfoldConfig(
        dashboard_callback="myapp.callbacks.business_metrics_callback"
    )
```
...
```python
def advanced_dashboard_callback(request: HttpRequest, context: Dict[str, Any]) -> Dict[str, Any]:
    """Advanced dashboard with charts and actions."""
    ...
    context["quick_actions"] = [
        ...
        },
        {
            ...
            "url": reverse('admin:system_backup'),
            "icon": "backup",
            "color": "warning"
        }
    ]
...
```python
from django_cfg.routing.callbacks import register_dashboard_callback

# Register multiple callbacks
@register_dashboard_callback("system_metrics")
def system_metrics_callback(request, context):
    """Add system metrics to dashboard."""
    # Implementation here
    return context

@register_dashboard_callback("user_analytics")
def user_analytics_callback(request, context):
    """Add user analytics to dashboard."""
    # Implementation here
    return context
