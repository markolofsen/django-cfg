...
class PaymentsSettings(BaseExtensionSettings):
    ...
    api_key: str = env.payments.api_key
```
...
|-------|------|-------------|
| `admin_enabled` | `bool` | Show in admin interface |
| `middleware_classes` | `list[str]` | Middleware classes to add to MIDDLEWARE |
| `navigation` | `Optional[NavigationSection]` | Admin navigation section |
...
| `url_prefix` | `Optional[str]` | URL path segment override |
| `url_namespace` | `Optional[str]` | Django URL namespace |
...
# Extension is enabled when the settings object exists
...
**2. Set in `.env`:**
```bash
# Nested structure: payments_api_keys.crypto_api_key
PAYMENTS_API_KEYS__CRYPTO_API_KEY=your-key-here
PAYMENTS_API_KEYS__CRYPTO_WEBHOOK_SECRET=your-secret-here
```
...
1. **Check `settings` variable exists:**
   ...
   settings = MyExtensionSettings()  # ← Must be present!
   ```
...
2. **Check variable naming:**
   ...
   PAYMENTS_API_KEYS__CRYPTO_API_KEY=key  # ✅ Correct
   PAYMENTS_API_KEYS.CRYPTO_API_KEY=key   # ❌ Wrong
   ```
...
```
/cfg/payments/          ← urls.py (main API)
/cfg/payments/admin/    ← urls_admin.py (admin API)
/cfg/payments/system/   ← urls_system.py (system API)
/cfg/payments/<custom>/ ← urls_<custom>.py (custom endpoints)

/api/                   ← Your app URLs (no conflict)
/admin/                 ← Django admin (no conflict)
```
...
2. Configure API keys:
   ```bash
   # .env
   PAYMENTS_API_KEYS__CRYPTO_API_KEY=your-key
   PAYMENTS_API_KEYS__CRYPTO_WEBHOOK_SECRET=your-secret
   ```
...
```python
class MyModuleSettings(BaseModuleSettings):
    enabled: bool = False
```
...
<Cards>
  <Cards.Card title="Frontend Extensions" href="/extensions/frontend-extensions" />
  <Cards.Card title="Migration Guide" href="/extensions/migration-guide" />
  <Cards.Card title="Browse Extensions" href="https://github.com/markolofsen/django-cfg" />
</Cards>
