...
- Optional-dependency entries (ngrok, import-export, Unfold) that fail with `ImportError` are logged at `DEBUG` level only — these failures are expected when the optional package is not installed.
