...
Create a Django-based SaaS platform with [type-safe YAML configuration](./configuration) using Django-CFG.
...
- ✅ Python 3.12+ installed ([Installation Guide](./installation))
...
```python
"""
Environment configuration with [Pydantic models](../../core/type-safety) and automatic env loading.
...
# - CORS: localhost on any port (regex, credentials supported)
...
class SaaSConfig(DjangoConfig):
    ...
    # Database from YAML URL (see /core/database for multi-database setup)
...
Update **core/environment/loader.py** (see [Cache Configuration](../../configuration/cache) for advanced options):
