...
description: Learn intro in Django-CFG. Step-by-step guide to django-cfg - type-safe django configuration with code examples, best practices, and production-ready configuration.
---
...
- ✅ **Zero-config** built-in apps (user management, background tasks)
...
Django-CFG Built-in:               Production-ready modules
```
...
**Challenge:** Integrate with Stripe (payments), OpenAI (AI), Redis (cache), PostgreSQL (data).
...
from django_cfg import (
    DjangoConfig, DatabaseConfig, EmailConfig, SpectacularConfig, DjangoRQConfig
)
from typing import Dict
...
class APIServiceConfig(DjangoConfig):
    ...
    redis_url: str = f"redis://{env.redis.host}:6379/0"
    # Django-CFG auto-creates cache from redis_url; for task queues use django_rq
...
**Result:** All integrations type-safe and validated. 40 lines vs 150+. Auto-generated API docs.

### 4. Startup MVP (Rapid Development)

**Challenge:** Build and deploy in 1 day. User auth, email notifications, background tasks.

**Traditional Django:** Install 8-10 packages, configure each one, wire everything together, write custom user model.
...
```mermaid
graph TB
    A["1. Define config class<br/>(inherits DjangoConfig)"] --> B["2. Instantiate config<br/>(validates fields, loads env vars)"]
    B --> C["3. Call get_all_settings()<br/>(generates Django settings dict)"]
    C --> D["4. Django loads with<br/>validated configuration"]
...
class MyConfig(DjangoConfig):
    ...
    # Cache - auto-created from redis_url (recommended)
    redis_url: str | None = "redis://localhost:6379/0"
    # Django-CFG automatically creates CacheConfig! ✨

    # Advanced: Manual cache override (optional)
    # cache_default: CacheConfig | None = None  # Only if custom backend needed
...
**New in Django-CFG 2.0: Extensions Architecture**

Your `DjangoConfig` can be even simpler with the new **modular extensions system**! Instead of configuring everything in one file, extensions now configure themselves automatically.

```python
# Modern approach - extensions auto-discovered!
class MyConfig(DjangoConfig):
    project_name: str = "My SaaS"
    databases: Dict[str, DatabaseConfig] = {...}
    project_apps: list[str] = ["core"]  # Only your apps
    # Extensions like payments, leads, support auto-register! ✨
```

[Learn about Extensions →](../../extensions) | [Migration Guide →](../../extensions/migration-guide)
</Callout>
...
class MyConfig(DjangoConfig):
    enable_accounts: bool = True     # Extended user management with OTP
    enable_pool_cleanup: bool = False # Database connection pool cleanup
    enable_drf_tailwind: bool = False # Tailwind CSS for DRF views
```
...
```mermaid
graph TB
    A[DjangoConfig<br/>Pydantic Model] --> B[Field Validation<br/>Pydantic]
    B --> C[Environment Variable<br/>Substitution]
    C --> D[Smart Defaults<br/>Application]
    D --> E[Django Settings<br/>Generation]
    E --> F[settings.py globals]
...
1. [Installation Guide](./installation) - Install and verify setup
2. [First Project](./first-project) - Create a project from scratch
...
- [Database Configuration](../../configuration) - Multiple databases, routing
- [Caching Setup](../../configuration) - Redis, in-memory, database caching
- [Background Tasks](../../features/modules/django-rq/overview) - Async task processing
...
**Ready to start?** → [Installation Guide](./installation)
