...
class TOTPService:
    ...
    def verify_code(
        ...
        # Never log the submitted code, the expected code (totp.now()), or the secret —
        # a log reader could replay the live OTP within the window (TOTP-001).
        logger.info(f"TOTP verify: device={device.id}, is_valid={is_valid}")
