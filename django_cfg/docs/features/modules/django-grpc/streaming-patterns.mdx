...
Never use `connection_timeout` on `anext()` calls in grpcio async streaming. This causes premature `StopAsyncIteration`. See [Troubleshooting](/features/modules/django-grpc/troubleshooting#critical-connection_timeout-bug) for details.
</Callout>
