...
class MyConfig(DjangoConfig):
    ...
    databases: dict[str, DatabaseConfig] = {
        "default": DatabaseConfig(
            ...
            name="db.sqlite3"
        ) if _env == "development" else DatabaseConfig(
    ...
    email: EmailConfig = EmailConfig(
        backend="console" if _env == "development" else "smtp",
        host=env.email.host if _env != "development" else "localhost",
    )
...
- ✅ Reads `ENV` environment variable
...
**Core Solutions:**
- **[Type-Safe Django Configuration](../../core/type-safety)** - Complete implementation guide
...
- **[Environment Detection](../../configuration/environment)** - Multi-environment patterns
...
**Advanced Setup:**
- **[Database Configuration](../../database)** - Multi-database setup
...
- **[Django Integration](../../core/django-integration)** - Framework integration patterns

### Features & Tools

**Built-in Features:**
- **[Built-in Apps Overview](../../features/built-in-apps/overview)** - Production-ready apps
- **[User Management](/features/built-in-apps/user-management)** - Accounts and authentication
