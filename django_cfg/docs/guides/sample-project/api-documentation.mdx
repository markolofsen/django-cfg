...
OpenAPI schema title and description are configured separately via `OpenAPIClientConfig`:
...
openapi_client: OpenAPIClientConfig = OpenAPIClientConfig(
    enabled=True,
    drf_title="Django-CFG Sample API",
    drf_description="Complete API for Django-CFG sample project",
    drf_version="1.0.0",
)
```

See [Configuration](./configuration) for complete setup.
