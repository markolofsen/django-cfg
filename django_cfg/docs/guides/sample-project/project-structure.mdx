...
│       └── 📥 loader.py             # Pydantic BaseSettings loader
...
Environment loader using `pydantic-settings` (`BaseSettings`). All configuration is loaded from environment variables and `.env` files — no YAML files.

**loader.py** - Defines pydantic BaseSettings models:
```python
...
Keep environment-specific settings in `.env` files (gitignored) or system environment variables:

```bash
# .env (development, gitignored)
DEBUG=true
DATABASE__URL="sqlite:///db/db.sqlite3"

# Production: system ENV or Docker/K8s secrets
DEBUG=false
