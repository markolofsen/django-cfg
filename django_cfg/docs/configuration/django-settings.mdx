...
class DjangoConfig(BaseModel):
    ...
    encryption: Optional[EncryptionConfig] = Field(
        default=None,
        description="API response encryption to prevent data scraping",
    )
    ...
    extra_authentication_classes: List[str] = Field(
        default_factory=list,
        description="Extra DRF authentication classes prepended before defaults",
    )
    ...
    geo: Optional[GeoConfig] = Field(
        default=None,
        description="Geographic and timezone configuration",
    )
    ...
    grpc_module: Optional[DjangoGrpcModuleConfig] = Field(
        default=None,
        description="D1-backed gRPC module configuration",
    )
    ...
    frontend_monitor: Optional[FrontendMonitorConfig] = Field(
        default=None,
        description="Frontend error monitoring configuration",
    )
    cloudflare: Optional[CloudflareConfig] = Field(
        default=None,
        description="Cloudflare D1 integration configuration",
    )
...
Builds an **absolute URL** for a media file from its storage path, handling both
relative and CDN-absolute `media_url` values automatically.

| Argument | Type | Description |
|----------|------|-------------|
| `path` | `str \| None` | Relative path as stored in a model field, e.g. `"avatars/user_42.jpg"`. Returns `fallback` if empty/None. |
| `fallback` | `str \| None` | Value returned when `path` is empty (default `None`). |

```python
# media_url = "/media/"  (relative — prepends api_url)
config.build_media_url("avatars/user_42.jpg")
# → "https://api.example.com/media/avatars/user_42.jpg"

# media_url = "https://cdn.example.com/media/"  (absolute CDN)
config.build_media_url("avatars/user_42.jpg")
...
```python
from django_cfg import get_media_url, build_media_url

# Auto-reads current config — use anywhere after Django is set up:
avatar_url = get_media_url(user.avatar)
logo_url   = get_media_url(org.logo, fallback="/static/default-logo.png")

# Explicit — useful in tests or code running before Django starts:
url = build_media_url(
    "logos/company.png",
...
| Argument | Type | Description |
|----------|------|-------------|
| `path` | `str` | URL path segment, e.g. `"apix/catalog_photos/photo/abc.jpg"`. Leading slash is stripped automatically. |
...
prefix = config.openapi_client.api_prefix  # e.g. "apix"
url = config.build_api_url(f"{prefix}/catalog_photos/photo/{photo_id}.jpg")
...
```python
from django_cfg import get_api_url, build_api_url

# Auto-reads current config:
url = get_api_url("apix/catalog_photos/photo/abc.jpg")
# → "https://api.example.com/apix/catalog_photos/photo/abc.jpg"

# Explicit:
url = build_api_url("apix/photo/abc.jpg", api_url="https://api.example.com")
```
...
```python
from django_cfg import DjangoConfig, DatabaseConfig, DRFConfig, UnfoldConfig
...
### Modules
- [**File Cleanup**](../features/modules/django-cleanup/overview) - Automatic file cleanup for FileField/ImageField
- [**Currency**](../features/modules/django-currency/overview) - Currency conversion and exchange rates
- [**Telegram**](../features/modules/django-telegram/overview) - Telegram bot notifications
- [**Unfold Admin**](../features/modules/django-unfold/overview) - Modern admin interface

### Integrations
- [**Django-RQ**](../features/modules/django-rq/overview) - Background task queue
- [**Centrifugo**](../features/modules/django-centrifugo) - WebSocket pub/sub
- [**gRPC**](../features/modules/django-grpc) - gRPC server configuration
- [**Ngrok**](../features/modules/django-ngrok/overview) - Development tunneling
- [**OAuth**](../features/integrations/oauth) - Social authentication
