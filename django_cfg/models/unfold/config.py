...
Includes both original UnfoldConfig and extended UnfoldDashboardConfig.
"""
...
class UnfoldConfig(BaseConfig):
    ...
    def get_default_navigation(self) -> List[Dict[str, Any]]:
    ...
    def get_default_site_dropdown(self) -> List[Dict[str, Any]]:
        """Get default site dropdown items."""
        return [
            {
                "icon": "language",
                "title": "Documentation",
    ...
    def to_django_settings(self) -> Dict[str, Any]:
        ...
        unfold_settings["SIDEBAR"] = {
            "show_search": self.show_search,
            "command_search": self.command_search,
            "show_all_applications": self.show_all_applications,
        }
        
        # Add navigation if configured, otherwise use defaults
        if self.navigation_items:
            unfold_settings["SIDEBAR"]["navigation"] = self.navigation_items
        else:
            unfold_settings["SIDEBAR"]["navigation"] = self.get_default_navigation()
        
        # Add site dropdown if configured, otherwise use defaults
        if self.site_dropdown_items:
            unfold_settings["SITE_DROPDOWN"] = self.site_dropdown_items
        else:
            unfold_settings["SITE_DROPDOWN"] = self.get_default_site_dropdown()
...
class UnfoldDashboardConfig(BaseModel):
    ...
    site_symbol: str = Field(default="admin_panel_settings", description="Site icon")
    ...
    def to_unfold_dict(self) -> Dict[str, Any]:
        ...
        if self.navigation_sections:
            ...
            for section in self.navigation_sections:
                ...
                for item in section.items:
                    ...
                    if item.badge:
                        nav_item["badge"] = item.badge
                    if item.permission:
                        nav_item["permission"] = item.permission
                    
                    nav_section["items"].append(nav_item)
                
                navigation.append(nav_section)
            
            sidebar_config["navigation"] = navigation
        ...
        if self.site_dropdown_items:
            dropdown = []
            for item in self.site_dropdown_items:
                dropdown.append({
                    "title": item.title,
                    "icon": item.icon,
                    "link": item.link,
                })
            base_config["SITE_DROPDOWN"] = dropdown
