...
class GroupManager:
    ...
    def _build_urlpatterns(self, group_name: str, apps: List[str]) -> list:
        ...
        """
        # --- django-cfg built-in apps and modules ---
        cfg_prefixes = ("django_cfg.apps.", "django_cfg.modules.")
        if all(any(a.startswith(p) for p in cfg_prefixes) for a in apps):
            patterns = []
            for app_name in apps:
                ...
                try:
                    ...
                    continue
                # Strip django_ prefix for modules: django_monitor -> monitor
                module_name = app_name.split('.')[-1].removeprefix("django_")
