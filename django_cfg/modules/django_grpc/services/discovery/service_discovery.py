...
from ..management.config_helper import get_grpc_module_config
from .utils import (
    is_grpc_service,
...
class ServiceDiscovery:
    ...
    Example:
        ```python
        from django_cfg.modules.django_grpc.services.discovery import ServiceDiscovery
    ...
    def __init__(self):
        ...
        self.config = get_grpc_module_config()

        if self.config:
            # enabled_apps drives auto-registration (replaces old auto_register_apps bool)
            self.auto_register = bool(self.config.enabled_apps)
            self.enabled_apps = list(self.config.enabled_apps)
            # custom_services not present in DjangoGrpcModuleConfig — use empty dict
            self.custom_services: dict = {}
        else:
    ...
    def _load_handler_hook(self, handlers_hook_path: str) -> Optional[Any]:
        ...
        if "{ROOT_URLCONF}" in handlers_hook_path:
            try:
                from ..management.config_helper import get_root_urlconf
