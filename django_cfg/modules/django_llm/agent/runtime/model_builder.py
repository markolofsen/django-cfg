...
*machinery*: reading the provider/key/endpoint from `django_llm`, building an
...
def resolve_provider() -> tuple[str, str, str | None]:
    """Return ``(provider_name, api_key, base_url)`` from ``django_llm``.
    ...
    # `modules.llm_router` was cmdop's sandbox/djangocfg switch — a host module that
    ...
    logger.error(
        ...
        "django_llm.get_api_keys(). The agent will fail on first call.",
        ", ".join(urls),
    )
...
def build_chat_model(model_name: str) -> OpenAIChatModel:
    """Build one chat model on whichever provider ``django_llm`` resolves."""
