...
import type { ProfilesProfilesRetrieveData, ProfilesProfilesRetrieveResponses } from "../../types.gen";
import { UserProfileSchema } from "../schemas/UserProfile";

type Result = ProfilesProfilesRetrieveResponses[keyof ProfilesProfilesRetrieveResponses];
...
export function useProfilesProfilesRetrieve(
  args: Omit<ProfilesProfilesRetrieveData, "url" | "body">,
  config?: HookConfig,
) {
  const { enabled = true, ...swrConfig } = config ?? {};
  const key = enabled ? ["profiles_profiles_retrieve", args] as const : null;
  return useSWR<Result>(
    key,
    async () => {
      const res = await Profiles.profilesProfilesRetrieve({ ...args, throwOnError: true } as never);
      ...
      if (!parsed.success) {
        ...
        console.warn(
          ...
          {
            operation: "profiles_profiles_retrieve",
            method: "GET",
