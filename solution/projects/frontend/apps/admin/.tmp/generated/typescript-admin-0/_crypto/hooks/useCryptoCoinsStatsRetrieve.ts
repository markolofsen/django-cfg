...
import type { CryptoCoinsStatsRetrieveData, CryptoCoinsStatsRetrieveResponses } from "../../types.gen";
import { CoinStatsSchema } from "../schemas/CoinStats";

type Result = CryptoCoinsStatsRetrieveResponses[keyof CryptoCoinsStatsRetrieveResponses];
...
export function useCryptoCoinsStatsRetrieve(
  args?: Omit<CryptoCoinsStatsRetrieveData, "url" | "body">,
  config?: HookConfig,
) {
  const { enabled = true, ...swrConfig } = config ?? {};
  const key = enabled ? ["crypto_coins_stats_retrieve", args ?? {}] as const : null;
  return useSWR<Result>(
    key,
    async () => {
      const res = await Crypto.cryptoCoinsStatsRetrieve({ ...(args ?? {}), throwOnError: true } as never);
      const data = res.data as Result;
      const parsed = CoinStatsSchema.safeParse(data);
      if (!parsed.success) {
        ...
        console.warn(
          ...
          {
            operation: "crypto_coins_stats_retrieve",
            method: "GET",
