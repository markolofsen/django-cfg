...
import type { TradingPortfoliosStatsRetrieveData, TradingPortfoliosStatsRetrieveResponses } from "../../types.gen";
import { PortfolioStatsSchema } from "../schemas/PortfolioStats";

type Result = TradingPortfoliosStatsRetrieveResponses[keyof TradingPortfoliosStatsRetrieveResponses];
...
export function useTradingPortfoliosStatsRetrieve(
  args?: Omit<TradingPortfoliosStatsRetrieveData, "url" | "body">,
  config?: HookConfig,
) {
  const { enabled = true, ...swrConfig } = config ?? {};
  const key = enabled ? ["trading_portfolios_stats_retrieve", args ?? {}] as const : null;
  return useSWR<Result>(
    key,
    async () => {
      const res = await Trading.tradingPortfoliosStatsRetrieve({ ...(args ?? {}), throwOnError: true } as never);
      const data = res.data as Result;
      const parsed = PortfolioStatsSchema.safeParse(data);
      if (!parsed.success) {
        ...
        console.warn(
          ...
          {
            operation: "trading_portfolios_stats_retrieve",
            method: "GET",
