...
import type { ProfilesProfilesStatsRetrieveData, ProfilesProfilesStatsRetrieveResponses } from "../../types.gen";
import { UserProfileStatsSchema } from "../schemas/UserProfileStats";

type Result = ProfilesProfilesStatsRetrieveResponses[keyof ProfilesProfilesStatsRetrieveResponses];
...
export function useProfilesProfilesStatsRetrieve(
  args?: Omit<ProfilesProfilesStatsRetrieveData, "url" | "body">,
  config?: HookConfig,
) {
  const { enabled = true, ...swrConfig } = config ?? {};
  const key = enabled ? ["profiles_profiles_stats_retrieve", args ?? {}] as const : null;
  return useSWR<Result>(
    key,
    async () => {
      const res = await Profiles.profilesProfilesStatsRetrieve({ ...(args ?? {}), throwOnError: true } as never);
      const data = res.data as Result;
      const parsed = UserProfileStatsSchema.safeParse(data);
      if (!parsed.success) {
        ...
        console.warn(
          ...
          {
            operation: "profiles_profiles_stats_retrieve",
            method: "GET",
