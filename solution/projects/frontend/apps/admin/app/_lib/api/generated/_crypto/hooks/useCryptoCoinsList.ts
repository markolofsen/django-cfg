...
type Result = CryptoCoinsListResponses[keyof CryptoCoinsListResponses];
...
export function useCryptoCoinsList(
  args?: Omit<CryptoCoinsListData, "url" | "body">,
  config?: HookConfig,
) {
  const { enabled = true, ...swrConfig } = config ?? {};
  const key = enabled ? ["crypto_coins_list", args ?? {}] as const : null;
  return useSWR<Result>(
    key,
    async () => {
      const res = await Crypto.cryptoCoinsList({ ...(args ?? {}), throwOnError: true } as never);
