...
type Result = CryptoExchangesListResponses[keyof CryptoExchangesListResponses];
...
export function useCryptoExchangesList(
  args?: Omit<CryptoExchangesListData, "url" | "body">,
  config?: HookConfig,
) {
  const { enabled = true, ...swrConfig } = config ?? {};
  const key = enabled ? ["crypto_exchanges_list", args ?? {}] as const : null;
  return useSWR<Result>(
    key,
    async () => {
      const res = await Crypto.cryptoExchangesList({ ...(args ?? {}), throwOnError: true } as never);
