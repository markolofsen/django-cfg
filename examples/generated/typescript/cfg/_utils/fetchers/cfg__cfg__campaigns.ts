 ...
 */
export async function getCfgNewsletterCampaignsList(
  params?: { page?: number; page_size?: number },
  client?: API
): Promise<PaginatedNewsletterCampaignList> {
  const api = client || getAPIInstance()

  const response = await api.cfg_campaigns.cfgNewsletterCampaignsList(params?.page, params?.page_size)
 ...
 * @path /cfg/newsletter/campaigns/
 */
export async function createCfgNewsletterCampaigns(
  data: NewsletterCampaignRequest,
  client?: API
): Promise<NewsletterCampaign> {
  const api = client || getAPIInstance()

  const response = await api.cfg_campaigns.cfgNewsletterCampaignsCreate(data)
 ...
 */
export async function getCfgNewsletterCampaignsById(
  id: number,
  client?: API
): Promise<NewsletterCampaign> {
  const api = client || getAPIInstance()

  const response = await api.cfg_campaigns.cfgNewsletterCampaignsRetrieve(id)
  return NewsletterCampaignSchema.parse(response)
}
 ...
 * @path /cfg/newsletter/campaigns/{id}/
 */
export async function updateCfgNewsletterCampaigns(
  id: number, data: NewsletterCampaignRequest,
  client?: API
): Promise<NewsletterCampaign> {
  const api = client || getAPIInstance()

  const response = await api.cfg_campaigns.cfgNewsletterCampaignsUpdate(id, data)
  return NewsletterCampaignSchema.parse(response)
}
 ...
 * @path /cfg/newsletter/campaigns/{id}/
 */
export async function deleteCfgNewsletterCampaigns(
  id: number,
  client?: API
): Promise<void> {
  const api = client || getAPIInstance()

  const response = await api.cfg_campaigns.cfgNewsletterCampaignsDestroy(id)
  return response
}
 ...
 */
export async function createCfgNewsletterCampaignsSend(
  data: SendCampaignRequest,
  client?: API
): Promise<SendCampaignResponse> {
  const api = client || getAPIInstance()

  const response = await api.cfg_campaigns.cfgNewsletterCampaignsSendCreate(data)
