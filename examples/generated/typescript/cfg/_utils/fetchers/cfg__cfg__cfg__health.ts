 ...
 * @path /cfg/health/drf/
 */
export async function getCfgHealthDrfById(
  client?: API
): Promise<HealthCheck> {
  const api = client || getAPIInstance()

  const response = await api.cfg__health.cfgHealthDrfRetrieve()
  return HealthCheckSchema.parse(response)
}

/**
 * getCfgHealthDrfQuickById
 *
 ...
 */
export async function getCfgHealthDrfQuickById(
  client?: API
): Promise<QuickHealth> {
  const api = client || getAPIInstance()

  const response = await api.cfg__health.cfgHealthDrfQuickRetrieve()
