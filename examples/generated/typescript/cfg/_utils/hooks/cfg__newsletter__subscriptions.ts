 ...
 */
export function useCreateDjangoCfgNewsletterSubscribe() {
  const { mutate } = useSWRConfig()

  return async (data: SubscribeRequest): Promise<SubscribeResponse> => {
    const result = await Fetchers.createDjangoCfgNewsletterSubscribe(data)

    // Revalidate related queries
    mutate('django-cfg-newsletter-subscribe')
 ...
 */
export function useCreateDjangoCfgNewsletterUnsubscribe() {
  const { mutate } = useSWRConfig()

  return async (data: UnsubscribeRequest): Promise<SuccessResponse> => {
    const result = await Fetchers.createDjangoCfgNewsletterUnsubscribe(data)
