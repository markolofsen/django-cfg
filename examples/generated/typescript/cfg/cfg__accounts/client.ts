...
export class CfgAccountsAPI {
  ...
  async otpRequestCreate(data: Models.OTPRequestRequest): Promise<Models.OTPRequestResponse> {
    const response = await this.client.request('POST', "/django_cfg_accounts/otp/request/", { body: data });
    return response;
  }
  ...
  async otpVerifyCreate(data: Models.OTPVerifyRequest): Promise<Models.OTPVerifyResponse> {
    const response = await this.client.request('POST', "/django_cfg_accounts/otp/verify/", { body: data });
    return response;
  }
