...
@click.option('--port', default='8000', help='Port to run server on')
@click.option('--host', default='127.0.0.1', help='Host to bind to')
@click.option('--domain', help='Custom ngrok domain')
@click.option('--no-ngrok', is_flag=True, help='Disable ngrok tunnel')
def runserver_ngrok(port, host, domain, no_ngrok):
    """Run Django development server with ngrok tunnel."""

    # kill all django processes
    os.system('pkill -f "manage.py runserver"')

    setup_django()
    cmd = ['manage.py', 'runserver_ngrok', f'{host}:{port}']
    if domain:
...
# === Dramatiq Task Processing Commands ===

@cli.command()
@click.option('--processes', '-p', default=None, type=int, help='Number of worker processes')
@click.option('--threads', '-t', default=None, type=int, help='Number of threads per process')
@click.option('--queues', '-q', default=None, help='Comma-separated list of queues')
@click.option('--log-level', default=None, help='Log level (DEBUG, INFO, WARNING, ERROR)')
def rundramatiq(processes, threads, queues, log_level):
    """Run Dramatiq background task workers."""
    setup_django()
    cmd = ['manage.py', 'rundramatiq']
    if processes:
...
@cli.command()
@click.option('--queue', '-q', default=None, help='Specific queue to check')
def task_status(queue):
    """Show Dramatiq task status and queue information."""
    setup_django()
    cmd = ['manage.py', 'task_status']
    if queue:
...
@cli.command()
@click.option('--queue', '-q', default=None, help='Specific queue to clear')
@click.option('--confirm', is_flag=True, help='Skip confirmation prompt')
def task_clear(queue, confirm):
    """Clear Dramatiq queues."""
    setup_django()
    cmd = ['manage.py', 'task_clear']
    if queue:
        cmd.extend(['--queue', queue])
    if confirm:
        cmd.append('--confirm')
...
def runserver_ngrok():
    """Entry point for runserver_ngrok command."""
    cli(['runserver-ngrok'])
...
def rundramatiq():
    """Entry point for rundramatiq command."""
    cli(['rundramatiq'])

def task_status():
    """Entry point for task-status command."""
    cli(['task-status'])

def task_clear():
    """Entry point for task-clear command."""
    cli(['task-clear'])
