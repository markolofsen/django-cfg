...
class SampleProjectConfig(DjangoConfig):
            ...
            navigation=[
                ...
                NavigationSection(
                    ...
                    items=[
                        ...
                        NavigationItem(title="Categories", icon=Icons.CATEGORY, link="admin:shop_category_changelist"),
                    ],
                ),
                NavigationSection(
                    title="Profiles",
    ...
    ngrok: Optional[NgrokConfig] = NgrokConfig(
        ...
        tunnel=NgrokTunnelConfig(
            schemes=["https"],  # HTTPS only for webhooks
            compression=True,
        ),
        auto_start=True,  # Auto-start with runserver_ngrok
        update_api_url=True,  # Update api_url when tunnel is active
