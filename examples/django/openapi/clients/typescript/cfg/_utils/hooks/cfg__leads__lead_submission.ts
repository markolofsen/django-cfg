 ...
 *
 * @method POST
 * @path /django_cfg_leads/leads/submit/
 */
export function useCreateDjangoCfgLeadsLeadsSubmit() {
  const { mutate } = useSWRConfig()

  return async (data: LeadSubmissionRequest): Promise<LeadSubmissionResponse> => {
    const result = await Fetchers.createDjangoCfgLeadsLeadsSubmit(data)

    // Revalidate related queries
    mutate('django-cfg-leads-leads-submit')
