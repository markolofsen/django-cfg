 ...
 */
export async function createCfgNewsletterSubscribe(
  data: SubscribeRequest,
  client?: API
): Promise<SubscribeResponse> {
  const api = client || getAPIInstance()

  const response = await api.cfg_subscriptions.cfgNewsletterSubscribeCreate(data)
 ...
 */
export async function getCfgNewsletterSubscriptionsList(
  params?: { page?: number; page_size?: number },
  client?: API
): Promise<PaginatedNewsletterSubscriptionList> {
  const api = client || getAPIInstance()

  const response = await api.cfg_subscriptions.cfgNewsletterSubscriptionsList(params?.page, params?.page_size)
 ...
 */
export async function createCfgNewsletterUnsubscribe(
  data: UnsubscribeRequest,
  client?: API
): Promise<SuccessResponse> {
  const api = client || getAPIInstance()

  const response = await api.cfg_subscriptions.cfgNewsletterUnsubscribeCreate(data)
