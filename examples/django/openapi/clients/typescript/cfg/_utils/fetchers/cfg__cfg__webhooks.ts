 ...
 */
export async function getCfgPaymentsWebhooksById(
  provider: string,
  client?: API
): Promise<WebhookResponse> {
  const api = client || getAPIInstance()

  const response = await api.cfg_webhooks.cfgPaymentsWebhooksRetrieve(provider)
 ...
 * @path /cfg/payments/webhooks/{provider}/
 */
export async function createCfgPaymentsWebhooks(
  provider: string, data: WebhookResponseRequest,
  client?: API
): Promise<WebhookResponse> {
  const api = client || getAPIInstance()

  const response = await api.cfg_webhooks.cfgPaymentsWebhooksCreate(provider, data)
  return WebhookResponseSchema.parse(response)
}
 ...
 */
export async function getCfgPaymentsWebhooksHealthById(
  client?: API
): Promise<WebhookHealth> {
  const api = client || getAPIInstance()

  const response = await api.cfg_webhooks.cfgPaymentsWebhooksHealthRetrieve()
 ...
 */
export async function getCfgPaymentsWebhooksProvidersById(
  client?: API
): Promise<SupportedProviders> {
  const api = client || getAPIInstance()

  const response = await api.cfg_webhooks.cfgPaymentsWebhooksProvidersRetrieve()
 ...
 */
export async function getCfgPaymentsWebhooksStatsById(
  params?: { days?: number },
  client?: API
): Promise<WebhookStats> {
  const api = client || getAPIInstance()

  const response = await api.cfg_webhooks.cfgPaymentsWebhooksStatsRetrieve(params?.days)
