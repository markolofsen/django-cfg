 ...
 */
export async function getCfgNewsletterNewslettersList(
  params?: { page?: number; page_size?: number },
  client?: API
): Promise<PaginatedNewsletterList> {
  const api = client || getAPIInstance()

  const response = await api.cfg_newsletters.cfgNewsletterNewslettersList(params?.page, params?.page_size)
 ...
 */
export async function getCfgNewsletterNewslettersById(
  id: number,
  client?: API
): Promise<Newsletter> {
  const api = client || getAPIInstance()

  const response = await api.cfg_newsletters.cfgNewsletterNewslettersRetrieve(id)
