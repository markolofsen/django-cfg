...
class CfgCentrifugoAdminApiAPI:
    ...
    async def server_auth_token_create(self) -> None:
        """
        Get connection token for dashboard

        Returns JWT token and config for WebSocket connection to Centrifugo.
        """
        url = "/cfg/centrifugo/admin/api/server/auth/token/"
        response = await self._client.post(url)
        response.raise_for_status()
        return None


    async def server_channels_create(self, data: CentrifugoChannelsRequestRequest) -> CentrifugoChannelsResponse:
        """
        List active channels

        Returns list of active channels with optional pattern filter.
        """
        url = "/cfg/centrifugo/admin/api/server/channels/"
        response = await self._client.post(url, json=data.model_dump())
        response.raise_for_status()
        return CentrifugoChannelsResponse.model_validate(response.json())


    async def server_history_create(self, data: CentrifugoHistoryRequestRequest) -> CentrifugoHistoryResponse:
        """
        Get channel history

        Returns message history for a channel.
        """
        url = "/cfg/centrifugo/admin/api/server/history/"
        response = await self._client.post(url, json=data.model_dump())
        response.raise_for_status()
        return CentrifugoHistoryResponse.model_validate(response.json())


    async def server_info_create(self) -> CentrifugoInfoResponse:
        """
        Get Centrifugo server info

        Returns server information including node count, version, and uptime.
        """
        url = "/cfg/centrifugo/admin/api/server/info/"
        response = await self._client.post(url)
        response.raise_for_status()
        return CentrifugoInfoResponse.model_validate(response.json())


    async def server_presence_create(self, data: CentrifugoPresenceRequestRequest) -> CentrifugoPresenceResponse:
        """
        Get channel presence

        Returns list of clients currently subscribed to a channel.
        """
        url = "/cfg/centrifugo/admin/api/server/presence/"
        response = await self._client.post(url, json=data.model_dump())
        response.raise_for_status()
        return CentrifugoPresenceResponse.model_validate(response.json())


    async def server_presence_stats_create(self, data: CentrifugoPresenceStatsRequestRequest) -> CentrifugoPresenceStatsResponse:
        """
        ...
        num_users).
        """
        url = "/cfg/centrifugo/admin/api/server/presence-stats/"
        response = await self._client.post(url, json=data.model_dump())
        response.raise_for_status()
